...
const AdminComponent = () => {
  ...
  const fetchTemporaryUsers = async () => {
    try {
      const response = await axios.post('http://localhost:8083/api/manager/temporary', {});
  ...
  const handleConfirm = async (actionType) => {
    try {
      ...
      const url = actionType === 'accept' ? 'http://localhost:8083/api/manager/accept-user' : 'http://localhost:8083/api/manager/reject-user';
