...
const BoardList = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch(`http://localhost:8083/api/board/get-post?page=${currentPage}&size=10`); // 예시로 size=10 추가
    ...
    if (post.isSecret) {
      try {
        ...
        if (password !== null) {
          ...
          const response = await fetch('http://localhost:8083/api/board/verify-password', {
            method: 'POST',
            headers: {
          ...
          // 사용자가 취소 버튼을 눌렀거나 아무것도 입력하지 않았을 때의 처리
        }
      } catch (error) {
